...
%type <formals> error_formal_list
...
feature
    ...
    | OBJECTID '(' error error_formal_list ')' ':' TYPEID '{' expr '}' ';'	{ yyclearin; $$=NULL; }
    ;
...
expr
    ...
    | OBJECTID '(' expr expr_list ')'					{ $$ = dispatch(object(idtable.add_string("self")),$1, append_Expressions(single_Expressions($3), $4)); }
    | IF expr THEN expr ELSE expr FI					{ $$ = cond($2, $4, $6); }
    | WHILE expr LOOP expr POOL         { $$ = loop($2, $4);}
    | '{' expr ';' expr_block_list '}'  { $$ = block(append_Expressions(single_Expressions($2), $4)); }
    | LET let_expr      { $$ = $2; }
    ...
    /*errors*/
    | '{' error ';' expr_block_list '}'					{ yyclearin; $$=NULL; }	
    ;
...
expr_block_list
    ...
    | expr_block_list expr ';'						{ $$ = append_Expressions($1, single_Expressions($2)); }
    ;
